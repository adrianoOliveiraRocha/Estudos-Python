...
class MyWidget(QWidget):
    def __init__(self):
        ...
        self.connect(self.roiSpin, SIGNAL("valueChanged(int)"), 
            self.roiDial.setValue)

        self.yrsSlide.valueChanged.connect(self.yrsSpin.setValue)
        self.connect(self.yrsSpin, SIGNAL("valueChanged(int)"), 
    ...
    def showEMI(self):
        ...
        emi = (loanAmount * rateInterest) * ( ( ( (1 + rateInterest) ** noMonths ) 
            / ( ( (1 + rateInterest) ** noMonths ) - 1) ))
